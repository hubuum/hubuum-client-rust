...
[ -x scripts/release-notes.sh ] || fail "scripts/release-notes.sh must be executable"
scripts/release-notes.sh "$release_ref" >/dev/null || fail "CHANGELOG.md must contain release notes for $release_version"
...
cargo run -p hubuum_reconcile --locked -- check >/dev/null || fail "generated resource code is stale"
